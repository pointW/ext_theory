...
env_group.add_argument('--env', type=str, default='close_loop_block_arranging', help='block_picking, block_stacking, brick_stacking, '
...
env_group.add_argument('--view_type', type=str, default='camera_center_xyz_rgbd_noGripper')
...
env_group.add_argument('--workspace_option', type=str, default='none')
env_group.add_argument('--planner_mix_stack', type=int, default=None)
env_group.add_argument('--planner_mix_sort', type=int, default=None)
env_group.add_argument('--bin_num', type=int, default=10)
env_group.add_argument('--bin_dist', type=str, default='uniform')
env_group.add_argument('--correct_ratio', type=float, default=None)

training_group = parser.add_argument_group('training')
training_group.add_argument('--alg', default='bc_con')
training_group.add_argument('--model', type=str, default='equi_d1')
...
training_group.add_argument('--planner_episode', type=int, default=400)
...
if env == 'close_loop_block_arranging':
    args.workspace_option = 'white_plane,trans_robot,custom'
elif env == 'close_loop_block_color_sort':
    args.workspace_option = 'white_plane,trans_robot'
workspace_option = args.workspace_option.split(",")
...
if env == 'close_loop_block_arranging':
    if (args.planner_mix_stack == None) or (args.planner_mix_sort == None):
        assert args.correct_ratio != None, "please provide correct_ratio"
        args.planner_mix_stack = 400 * args.correct_ratio
        args.planner_mix_sort = 400 - args.planner_mix_stack
    else:
        assert (args.planner_mix_stack != 0) or (args.planner_mix_stack != 0), "planner num should not be both zero"
        

planner_mix_stack = args.planner_mix_stack
planner_mix_sort = args.planner_mix_sort
...
env_config = {'workspace': workspace, 'max_steps': max_episode_steps, 'obs_size': heightmap_size,
              ...
              'workspace_option': workspace_option, 'bin_num': bin_num, 'bin_dist': bin_dist,
              'planner_mix_stack': planner_mix_stack, 'planner_mix_sort': planner_mix_sort}
